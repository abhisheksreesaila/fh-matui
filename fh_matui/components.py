...
def FAQItem(question: str, answer: str, question_cls: str = '', answer_cls: str = ''):
    ...
    return Details(
        Summary(Article(Nav(Div(question, cls=f"max bold {question_cls}".strip()), I("expand_more")), cls="round primary no-elevate")),
        Article(P(answer, cls=f"secondary-text {answer_cls}".strip()), cls="round border padding"))

def FAQList(title: str, faqs: list, cls: str = ''):
    """Render a list of FAQs from list of dicts with 'question' and 'answer' keys."""
    if not isinstance(faqs, list): raise TypeError("FAQList: `faqs` must be a list of dicts")
    items = []
    for i, faq in enumerate(faqs):
        if not isinstance(faq, dict): raise TypeError(f"FAQList: faqs[{i}] must be a dict")
        if "question" not in faq or "answer" not in faq: raise KeyError(f"FAQList: faqs[{i}] must have keys 'question' and 'answer'")
        items.append(FAQItem(faq["question"], faq["answer"]))
    items_container = Div(*items, style="display:flex; flex-direction:column; gap:1rem;")
    return Section(H3(title, cls="medium"), items_container, cls=f"responsive {cls}".strip())
